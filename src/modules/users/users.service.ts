...
import { generateOtp, isPhoneNumber, validateEmail } from 'src/common/utils/checker.helper';
...
export class UsersService {
  ...
  async registerAdmin(createAdminDto: CreateAdminDto) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const existingEmail = email
      ? await this.AdminModel.findOne({ email: email })
      : null;
    console.log(existingEmail);
    const isUserExist = await this.findOneWithEmailorUserNameAdmin(username);
