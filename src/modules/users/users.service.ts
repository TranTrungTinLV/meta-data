...
export class UsersService {
  ...
  async sendForgotPasswordOtp(body: ForgotPasswordDto) {
    let user = null;
    if (body.type === ERole.USER || body.type === ERole.CUSTOMER) {
      user = await this.UserModel.findOne({ email: body.email });
    }
    if (body.type === ERole.ADMIN) {
      user = await this.AdminModel.findOne({ email: body.email });
    }
    if (!user) {
    ...
    //random number otp
    const otp = await generateOtp();
    await this.cacheService.set(`OTP:${body.email}`, otp, 60);
    //hash otp
