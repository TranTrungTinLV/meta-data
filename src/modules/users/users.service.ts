...
export class UsersService {
  ...
  async registerUser(signUpDto: CreateRegistorDto) {
    ...
    if(phone && !isPhoneNumber(phone)){
      throw new HttpException('số điện thoại không hợp lệ vui lòng kiểm tra lai6', HttpStatus.BAD_REQUEST);
  ...
  try{
     await this.sendEmail.add(
      ...
      //xoá dữ liệu sao 1p
  await this.sendEmail.add('deletePasswordReset',{userId: user._id},{delay: 60})
