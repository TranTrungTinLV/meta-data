...
import { delay } from "src/common/utils/time.util";
...
export class UploadsService {
  ...
  withMediaPathOfSubject(payload: IImagePayload): string {
    const PathDesViaEnterpriseStrategies = new Map([
      ...
      [`${EDirUpload.METADATA}`, `${payload.dir}`],
    ]);
    ...
    return withTransaction(this.connection, async (session: any) => {
      ...
      for (
        ...
        if (allFieldsHashValue && !assetExits) {
          ...
          this.logger.log(row);
          this.logger.debug(
            `Process - Insert metadata: insert metadata ${asset.name}`
          );
        }
      }
    });
  ...
  async getAllMetadata() {
    let data: any = await this.metadataModel.find().populate("category_id");
    data = data.map((item) => {
      return {
        "Danh mục": item.category_id["name"],
        "Mã vật tư": item.code,
        "Tên vật tư": item.name,
        "Quy cách": item.specification.join(""),
        "Đơn vị tính": item.unit,
        "Tiêu chuẩn": item.standard.join(""),
        "Ghi chú": item.note,
