...
export class CategoryService {
    ...
    async findAllWithDetails(): Promise<Category[]> {
      ...
      for(let category of categories) {
        const children = await this.categoryModel.find({children: category._id})
    ...
    async findProductsByCategory(categoryId: string): Promise<Category> {
      ...
      return category.populate('products','code name detail specification images');
    }
    ...
    async getAllCategoriesWithFull(): Promise<Category[]> {
        ...
        const rootCategories = await this.categoryModel.find({ category_parent_id: { $exists: false } }).exec();
        
        // Xử lý đệ quy cho mỗi danh mục gốc để lấy toàn bộ cây danh mục
        const allCategories = await Promise.all(
            rootCategories.map(rootCategory => this.populateChildrenCategoryandProducts(rootCategory._id.toString()))
        );
    ...
    async populateChildrenCategoryandProducts(id: string): Promise<Category> {
      if (!id) {
        console.error("ID không được cung cấp cho populateChildrenCategoryandProducts.");
        throw new BadRequestException("ID không được cung cấp.");
    }
    console.log(`Bắt đầu truy vấn cho danh mục với id: ${id}`);
    const objectId = new mongoose.Types.ObjectId(id);

      const category = await this.categoryModel.findById(objectId).populate('products');
      

    if (!category) {
        throw new BadRequestException(`Không tìm thấy danh mục với id ${id}`);
    }
    console.log(`Tìm thấy danh mục: ${category.name}, tiếp tục xử lý danh mục con`);
    ...
    const result = await this.categoryModel.aggregate([
        ...
        {
            $addFields: {
                "childrenDetails.productCount": { $size: "$childrenDetails.products" }
            }
        },
        {
            $group: {
      ...
      // for (let child of aggregatedCategory.children) {
        for (let i = 0; i < aggregatedCategory.children.length; i++) {
          if (aggregatedCategory.children[i]._id) {
            //đệ quy
            aggregatedCategory.children[i] = await this.populateChildrenCategoryandProducts(aggregatedCategory.children[i]._id.toString());
        }
    }
    return aggregatedCategory;
      ...
      throw new NotFoundException(`Không thể truy vấn thông tin chi tiết cho danh mục với id ${id}`);
  }
    }    
