...
export class ProductService {
  ...
  async create(
    ...
    const existsElastic = await this.metadataService.indexExists(index);
    let body = {
    ...
    if (!existsElastic) {
      console.log("existsElastic",existsElastic)
      await this.metadataService.createIndex(
        index,
        body
  ...
  async findAllProducts(
    ...
    if (filter.name) {
      ...
      const paginatedResult = await this.productModelpag.paginate(query, options);
       // Kiểm tra kết quả và xử lý kịch bản không tìm thấy sản phẩm
    if (paginatedResult.docs.length === 0) {
      throw new NotFoundException('Không tìm thấy sản phẩm nào với bộ lọc hiện tại.');
    }
