...
export class ProductService {
  ...
  async create(
    ...
    //checkduplicate name or alternativeName
    //if Duplicate Name it check alternativeName
    //or Duplicate alternativeName and name it Duplicate
    // const existingName = await this.productModel.findOne({
    //   $or: [
    //     {
    //       name: createProductDto.name,
    //     },
    //     // { alternativeName: createProductDto.alternativeName },
    //     // { name: { $in: createProductDto.alternativeName } },
    //     // { alternativeNames: { $in: createProductDto.alternativeName } },
    //     { code: createProductDto.code }
    //   ],
    // });

    // if (!existingName) {
    //   throw new BadRequestException('Name or alterName it existing!');
    // }
  ...
  async findAllProducts(
    ...
    if (filter.name) {
      ...
      const options = {
        page: filter.page || 1,
        // limit: filter.limit || NaN,
        sort: { createAt: -1 },
        populate: 'category_id',
      };
      // console.log('limit', options.limit);
      // if (options.limit = NaN) {
