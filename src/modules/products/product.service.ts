...
export class ProductService {
  constructor(
    ...
    // ** Paginate
    @InjectModel(Product.name)
    private readonly productModelPaginate: PaginateModel<Product>,
  ) {}
  ...
  async findAllProducts(
  ...
  ): Promise<PaginateResult<Product> | Product[]> {
    let query = {};
    // if(filter.name){
    ...
    if (filter.name) {
      ...
      return this.productModelPaginate.paginate(query, options);
    } else {
